    ...
    (condition-case err
        (if (condition-case nil (sqlite-select db "SELECT 1 FROM notes LIMIT 1") (error nil))
            ...
            (let ((rows (sqlite-select db "SELECT title, properties FROM notes WHERE properties IS NOT NULL")))
              (dolist (row rows)
                (let* ((title-raw (car row))
                       (title (if (stringp title-raw) 
                                  (condition-case nil (read title-raw) (error title-raw))
                                title-raw))
                       (props-raw (cadr row))
                       (props-str (if (stringp props-raw) 
                                      (condition-case nil (read props-raw) (error props-raw))
                                    props-raw))
                       ...
                       (key (let ((refs (cl-loop for (k . v) in props
                              ...
                              (cond
                               ...
                               (t (unless (string-match-p "^http" refs) refs))))))
                  (when key
